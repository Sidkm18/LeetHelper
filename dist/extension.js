...
function activate(context) {
    ...
    // Tree View Provider
    const treeProvider = new treeProvider_1.LeetCodeTreeProvider();
    ...
    context.subscriptions.push(vscode.commands.registerCommand('leethelp.searchProblem', async () => {
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                if (selected) {
                    vscode.commands.executeCommand('leethelp.openProblem', selected.formattedProblem);
                }
            }
            catch (error) {
    ...
    // CodeLens Provider
    const codeLensProvider = new codelensProvider_1.LeetCodeCodeLensProvider();
    context.subscriptions.push(vscode.languages.registerCodeLensProvider([{ scheme: 'file', language: 'python' }, { scheme: 'file', language: 'cpp' }, { scheme: 'file', language: 'java' }, { scheme: 'file', language: 'javascript' }, { scheme: 'file', language: 'typescript' }, { scheme: 'file', language: 'go' }], codeLensProvider));
    // Sign In Command
    ...
    context.subscriptions.push(vscode.commands.registerCommand('leethelp.openProblem', async (question) => {
        try {
            vscode.window.withProgress({
                location: vscode.ProgressLocation.Notification,
                ...
                webviewPanel_1.LeetCodeWebview.createOrShow(context.extensionUri, detailedQuestion);
...
function getExtension(langSlug) {
    const map = {
        'cpp': '.cpp',
        'java': '.java',
        'python3': '.py',
